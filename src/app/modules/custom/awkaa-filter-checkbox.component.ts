...
@Component({
  selector: 'awkaa-filter-checkbox',
  templateUrl: './awkaa-filter-checkbox.component.html',
  styleUrls: ['./awkaa-filter-checkbox.component.css'],
  host: {
   '(window:resize)': 'onResize($event)'
  }
})
export class AwkaaFilterCheckboxComponent implements OnInit {
  ...
  setDefaultValue(){
    ...
    for (let i = 0; i <  this.filter['dataSource'].length; i++) {
      this.filter['value'][i] = false ;
    }
  }
  getState(){
  ...
  getStateWithUrlParam(){
    ...
    //console.log('-.+');
    //console.log(this.filter['valueFormatted']);
    //console.log('-.,');

    for (let i = 0; i <  this.filter['valueFormatted'].length; i++) {
      let val = this.filter['valueFormatted'][i] ;
  ...
  setStateFromUrl( fragment  : string ){
     ...
     let urlParam  = this.filter['urlParam'] ! ;
      ...
      {
          let values = fragmentData[urlParam].split(',') ;

          for (let i = 0; i <  this.filter['dataSource'].length; i++) {

            //this.filter['value'][i] = false ;
            let val = this.filter['dataSource'][i][this.optionId] ;

             if( values.indexOf(val) != -1 )
            {
              this.filter['value'][i] = true ;
             }
          }
      }
      ...
      for (let i = 0; i <  this.filter['dataSource'].length; i++) {
        ...
        {
          this.filter['valueFormatted'][i] = val ;
        }
        else {
          this.filter['valueFormatted'][i] = "" ;
        }
      }
  }
